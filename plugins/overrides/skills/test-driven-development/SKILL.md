...
- **Verifying expected dependency behavior, before writing the assertion** — when the test would assert how an Elixir stdlib / Hex dep behaves, look it up via Tidewave's `get_docs` / `search_package_docs` (or Context7 for non-Hex deps) instead of guessing. For runtime confirmation use Tidewave's `project_eval` against the live app — far more reliable than ad-hoc `iex` snippets.

This applies whether you're test-driving new code, regression-testing a bug fix, or characterising existing behavior before a refactor.
